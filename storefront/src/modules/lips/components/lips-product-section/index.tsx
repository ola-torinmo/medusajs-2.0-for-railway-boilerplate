// src/modules/lips/components/lips-product-section/index.tsx
"use client"

import React, { useState, useMemo } from "react"
import { HttpTypes } from "@medusajs/types"
import { Text } from "@medusajs/ui"
import ProductPreview from "@modules/products/components/product-preview"
import { 
  LipsCategory, 
  FilterType, 
//   ProductWithTags 
} from "@modules/lips/types"

type LipsProductSectionProps = {
  products: HttpTypes.StoreProduct[]
  region: HttpTypes.StoreRegion
}

const LipsProductSection: React.FC<LipsProductSectionProps> = ({ products, region }) => {
  const [filter, setFilter] = useState<FilterType>("all")

  const categories: LipsCategory[] = useMemo(() => [
    { 
      id: "all", 
      name: "All Lips", 
      count: products.length 
    },
    { 
      id: "lipstick", 
      name: "Lipsticks", 
      count: products.filter(p => 
        p.tags?.some((tag: any) => tag.value?.toLowerCase().includes('lipstick')) ||
        p.title?.toLowerCase().includes('lipstick')
      ).length 
    },
    { 
      id: "lipgloss", 
      name: "Lip Gloss", 
      count: products.filter(p => 
        p.tags?.some((tag: any) => tag.value?.toLowerCase().includes('gloss')) ||
        p.title?.toLowerCase().includes('gloss')
      ).length 
    },
    { 
      id: "lipcare", 
      name: "Lip Care", 
      count: products.filter(p => 
        p.tags?.some((tag: any) => 
          tag.value?.toLowerCase().includes('balm') || 
          tag.value?.toLowerCase().includes('care')
        ) ||
        p.title?.toLowerCase().includes('balm') ||
        p.title?.toLowerCase().includes('care')
      ).length 
    },
  ], [products])

  const filteredProducts: HttpTypes.StoreProduct[] = useMemo(() => {
    if (filter === "all") return products
    
    return products.filter(product => {
      const title = product.title?.toLowerCase() || ""
      const hasTag = (searchTerm: string) => 
        product.tags?.some((tag: any) => tag.value?.toLowerCase().includes(searchTerm)) ||
        title.includes(searchTerm)

      switch (filter) {
        case "lipstick":
          return hasTag('lipstick')
        case "lipgloss":
          return hasTag('gloss')
        case "lipcare":
          return hasTag('balm') || hasTag('care')
        default:
          return true
      }
    })
  }, [products, filter])

  const handleFilterChange = (newFilter: FilterType): void => {
    setFilter(newFilter)
  }

  return (
    <div className="pt-16 pb-1 bg-[#F9F5F2]">
      <div className="content-container">
        {/* Section Header */}
        <div className="mb-12 text-center">
          <h2 className="text-[26px] font-bold text-gray-900 mb-2 uppercase tracking-wide">
            CLASSY AND ELEGANT
          </h2>
          <p className="text-gray-600 max-w-2xl mx-auto">
            Explore our lip products that combine rich pigmentation with nourishing ingredients.
          </p>
        </div>

        {/* Filter Buttons - only show if we have products */}
        {/* {products.length > 0 && (
          <div className="flex flex-wrap gap-3 mb-12 justify-center">
            {categories.map((category) => (
              <button
                key={category.id}
                onClick={() => handleFilterChange(category.id as FilterType)}
                className={`px-6 py-3 rounded-full text-small-regular transition-all duration-200 ${
                  filter === category.id
                    ? "bg-[#B07A5D] text-white shadow-lg"
                    : "bg-white text-gray-700 hover:bg-gray-50 border border-gray-200 hover:border-[#B07A5D] hover:text-[#B07A5D]"
                }`}
                type="button"
              >
                {category.name} ({category.count})
              </button>
            ))}
          </div>
        )} */}

        {/* Mobile Slider - Shows only on mobile */}
        <div className="block sm:hidden mb-8">
          <div className="flex gap-4 overflow-x-auto scrollbar-hide px-4 py-2">
            {filteredProducts.slice(0, 8).map((product) => (
              <div key={product.id} className="flex-shrink-0 w-[280px] group">
                <ProductPreview
                  product={product}
                  region={region}
                  isFeatured
                />
              </div>
            ))}
          </div>
        </div>

        {/* Desktop/Tablet Grid - Shows on sm screens and up */}
        <div className="hidden sm:grid sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8 p-[5px]">
          {filteredProducts.slice(0, 8).map((product) => (
            <div key={product.id} className="group">
              <ProductPreview
                product={product}
                region={region}
                isFeatured
              />
            </div>
          ))}
        </div>

        {/* Empty State */}
        {/* {filteredProducts.length === 0 && products.length > 0 && (
          <div className="text-center py-16">
            <div className="text-6xl mb-4">💋</div>
            <h3 className="text-xl font-semibold text-gray-900 mb-2">No products found</h3>
            <Text className="text-gray-500 mb-6">
              No products found in the {categories.find(c => c.id === filter)?.name} category.
            </Text>
            <button
              onClick={() => handleFilterChange("all")}
              className="px-6 py-2 bg-[#B07A5D] text-white rounded-full hover:bg-[#9A6B4F] transition-colors"
            >
              View All Products
            </button>
          </div>
        )} */}

        {/* No Products State */}
        {products.length === 0 && (
          <div className="text-center py-16">
            <div className="text-6xl mb-4">💄</div>
            <h3 className="text-xl font-semibold text-gray-900 mb-2">Coming Soon</h3>
            <Text className="text-gray-500">
              We're working on adding amazing lip products to our collection.
            </Text>
          </div>
        )}
      </div>
    </div>
  )
}

export default LipsProductSection